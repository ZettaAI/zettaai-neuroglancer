...
export function ListboxDropdown({
  ...
  const handleCursorKey = (e: KeyboardEvent): boolean => {
    switch (e.key) {
      case "ArrowDown":
        e.preventDefault();
        moveCursor(1);
  ...
  const onSearchInput = (e: Event) => {
    const nextQuery = (e.currentTarget as HTMLInputElement).value;
    setQuery(nextQuery);
    const nextVisible =
      nextQuery === ""
        ? options
  ...
  const optionRows = visibleOptions.map((option, index) => {
    ...
    return (
      ...
      >
        <span class="neuroglancer-listbox-dropdown-check">
          {isSelected && <Check size={14} aria-hidden="true" />}
        </span>
        <span
          class="neuroglancer-listbox-dropdown-option-label"
          title={option.label}
        >
          {option.label}
        </span>
  ...
  return (
    <span ref={wrapperRef} class="neuroglancer-listbox-dropdown">
      <button
        ref={triggerRef}
        type="button"
        class="neuroglancer-listbox-dropdown-trigger"
        disabled={disabled}
        ...
        onClick={toggle}
      >
        <span class="neuroglancer-listbox-dropdown-summary">{summary}</span>
        <ChevronDown
          class="neuroglancer-listbox-dropdown-caret"
          size={16}
          aria-hidden="true"
        />
      </button>
      {open &&
        ...
        createPortal(
          <div
            ref={panelRef}
            class="neuroglancer-listbox-dropdown-panel"
            ...
            {filterable === true ? (
              <>
                <input
                  ref={searchRef}
                  type="text"
                  class="neuroglancer-listbox-dropdown-search"
                  placeholder="Search…"
                  aria-label={
