...
describe("joinBackendUrl", () => {
  it("appends a relative path onto the base path", () => {
    expect(joinBackendUrl("https://h/segmentation", "propagate_mask")).toBe(
      "https://h/segmentation/propagate_mask",
    );
  });

  it("treats a leading-slash path as relative to the base path", () => {
    expect(joinBackendUrl("https://h/segmentation", "/propagate_mask")).toBe(
      "https://h/segmentation/propagate_mask",
    );
  });
  ...
  it("preserves a query already on the base (e.g. host-baked subportal_id)", () => {
    expect(
      joinBackendUrl("https://h/seg?subportal_id=abc", "propagate_mask"),
    ).toBe("https://h/seg/propagate_mask?subportal_id=abc");
  });

  it("merges base and path query, with the path winning on conflict", () => {
    expect(
      joinBackendUrl(
        "https://h/seg?subportal_id=abc&keep=1",
        "x?subportal_id=z",
      ),
    ).toBe("https://h/seg/x?subportal_id=z&keep=1");
  });
});
...
describe("BackendClient", () => {
  ...
  it("fetches the joined URL and applies authorize", async () => {
    ...
    registerBackendEndpoint({
      baseUrl: "https://h/segmentation",
      authorize,
    });

    const client = new BackendClient();
    await client.request("/propagate_mask", { method: "POST" });

    expect(authorize).toHaveBeenCalledOnce();
    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe("https://h/segmentation/propagate_mask");
    expect(
  ...
  it("posts multipart bodies without forcing a Content-Type", async () => {
    ...
    const client = new BackendClient();
    await client.postMultipart("/propagate_mask", form);
